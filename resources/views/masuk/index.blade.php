    ...
    <div class="card card-docs flex-row-fluid mb-2">
        <div class="card-header d-flex justify-content-between">
            ...
            <div class="d-flex flex-stack">
                <a href="{{ route('masuk.print') }}" class="btn btn-secondary">
                    Cetak Data
                </a>
                <a type="button" class="btn btn-primary ms-2" href="{{ route('masuk.create') }}">
                    Tambah Data
                </a>
            </div>
        </div>
        <div class="card-body pt-0">
            <table id="masuk-table" class="table align-middle table-row-dashed fs-6 gy-5">
                <thead>
                    <tr class="fw-semibold fs-6 text-muted">
                        <th class="text-start min-w-100px">No</th>
                        <th class="text-start min-w-70px">Tanggal </th>
                        <th class="text-start min-w-70px">Admin</th>
                        <th class="text-start min-w-70px">Bahan Produksi</th>
...
@push('scripts')
    <script>
        var datatable = $('#masuk-table').DataTable({
            processing: true,
            serverSide: true,
            ordering: true,
            stateSave: false,
        ...
        $(document).on("click", ".delete-confirm", function(e) {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    e.preventDefault();
                    var id = $(this).data("id");
                    var route = "{{ route('masuk.destroy', ':id') }}";
