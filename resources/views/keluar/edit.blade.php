...
@section('content')
    <div class="card card-docs flex-row-fluid mb-2">
        <div class="card-body fs-6 text-gray-700">
            ...
            <form action="{{ route('keluar.update', $produksi->id) }}" method="POST" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                        ...
                        <div class="col-lg">
                            <select name="customer_id" class="form-control @error('customer_id') is-invalid @enderror">
                                ...
                                @foreach($pelanggans as $pelanggan)
                                    <option value="{{ $pelanggan->id }}" {{ $pelanggan->id == old('customer_id', $produksi->customer_id) ? 'selected' : '' }}>
                                        {{ $pelanggan->name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="col-lg">
                            <select name="product" class="form-control @error('product') is-invalid @enderror">
                                ...
                                @foreach ($products as $product)
                                    <option value="{{ $product->product }}" {{ $product->product == old('product', $produksi->product) ? 'selected' : '' }}>
                                        {{ $product->product }}
                                    </option>
                                @endforeach
                            </select>
                    ...
                    <div class="row mb-5">
                        ...
                        <div class="col-lg">
                            <input type="text" name="harga" class="form-control @error('harga') is-invalid @enderror"
                                value="{{ old('harga', $produksi->harga) }}" readonly />
                            @error('harga')
                                <div class="invalid-feedback">
                                    {{ $message }}
                                </div>
                    ...
                    <div class="row mb-5">
                        ...
                        <div class="col-lg">
                            <input type="number" name="jumlah" class="form-control @error('jumlah') is-invalid @enderror"
                                value="{{ old('jumlah', $produksi->jumlah) }}" readonly />
                            @error('jumlah')
                                <div class="invalid-feedback">
                                    {{ $message }}
                                </div>
                    ...
                    <div class="row mb-5">
                        ...
                        <div class="col-lg">
                            <input type="number" name="jumlah_beli" class="form-control @error('jumlah_beli') is-invalid @enderror"
                                value="{{ old('jumlah_beli', $produksi->jumlah_beli) }}" />
                            @error('jumlah_beli')
                                <div class="invalid-feedback">
                                    {{ $message }}
                                </div>
                ...
                <div class="card-footer d-flex justify-content-end py-6 px-9">
                    <a href="{{ route('keluar.index') }}" type="reset" class="btn btn-light btn-active-light-primary me-2">Batalkan</a>
