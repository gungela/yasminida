...
class SupplierController extends Controller
{
    ...
    {
        if ($request->ajax()) {
            $customer = Customer::where('status','supplier')->get();
                ...
                ->addColumn('actions', function ($item) {
                    ...
                    <div class="dropdown-menu menu menu-sub menu-sub-dropdown menu-column menu-rounded menu-gray-800 menu-state-bg-light-primary fw-semibold w-100px py-4" data-kt-menu="true">
                        <div class="menu-item px-3">
                            <a href="' . route('supplier.edit', $item->id) . '" class="menu-link px-3">
                                Edit Profile
                            </a>
                        </div>
                        <div class="menu-item px-3">
    ...
    public function store(Request $request)
    {
        try {
            ...
            return to_route('supplier.index', $supplier)->with('success', 'Supplier created successfully');
        } catch (\Throwable $th) {
    ...
    {
        $supplier = Customer::whereId($id)->firstOrFail();
    ...
    public function update(Request $request,$id)
    {
        try {
            ...
            return to_route('supplier.index')->with('success', 'Supplier Update successfully');
        } catch (\Throwable $th) {
            return back()->with('error', 'Error: ' . $th->getMessage());
        }
    }
